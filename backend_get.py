...
cgitb.enable(display=0, logdir="tmp/out.html")
...
if str_type <> 2 and form['testfile'].filename:
    ...
    #if input_file <> None:
        #cmd2 = 'dsub --project ' + str_data_1 + ' --zones "us-*"   --logging '+ log_file  +'   --input BAM=' + input_file + '  --output BAI=' + output_file + ' --image ' + image_file + ' --command "samtools index ${BAM} ${BAI}"'
...
if state1 == 1:
    ...
    print '<Br></Br>'
    print 'Output_file: &nbsp&nbsp  <input class="col-xs-6 col-sm-4" type="text" name="input_file" />'
    print '<Br><Br>'
